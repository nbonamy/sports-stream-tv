...
function waiting() {
  ...
  // playable data buffered. That is not a playback interruption.
  if (
    mode.value === "unavailable" ||
    !video.value ||
    video.value.paused ||
...
async function key(e: KeyboardEvent) {
  ...
  if (e.key === "Escape") {
    ...
    escapePending = true;
    try {
      const wasFullscreen = await api.fullscreen(false);
