...
async function back() {
  ...
  if (route.value.type === "player") await window.sports.fullscreen(false);
  ++loadGeneration;
  history.value.pop();
  await fetchRoute();
  await focusRoute();
...
async function fetchRoute(refresh = false) {
  ...
  if (r.type === "schedule" && (refresh || !cache.value[r.sport!.id])) {
    loading.value = true;
    try {
      const data = await window.sports.events(r.sport!.id);
    ...
    loading.value = true;
    try {
      const data = await window.sports.countries();
          ...
          >
            ...
            >
              ...
              <img
                ...
                class="league-icon"
                :src="`sports://app/league?url=${encodeURIComponent(event.leagueIconUrl)}`"
                alt=""
                @error="
      ...
      <template v-else-if="route.type === 'event'">
        ...
        <div class="channel-list">
          <button
            ...
            @click="play(c)"
          >
            <span class="play-triangle">▶</span>{{ c.name }}<Chevron right />
          </button>
        </div>
      </template>
      <template
